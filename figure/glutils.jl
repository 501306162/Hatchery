...
function loginit(filename::String)
...
function logadd(filename::String, message::String)
...
function logerror(filename::String, message::String)
    logfile = open(filename, "a")
    println(logfile, message)
...
function startgl()
    ...
    # create window
    global window = GLFW.CreateWindow(glfwWidth, glfwHeight, "Figure")
    if window == C_NULL
...
# create shader
function createshader(source::String, shaderType::GLenum)
    logadd("gl.log", string("creating shader from ", source, "..."))
    const shader = readstring(string(dirname(@__FILE__), "/", source))
...
# create program
function createprogram(vertID::GLuint, fragID::GLuint; geomID::GLuint=GLuint(0))
    shaderProgramID = glCreateProgram()
    logadd("gl.log", string("creating program ", shaderProgramID, " attaching shaders: ", vertID, " and ", fragID, "..."))
    if geomID != 0
        logadd("gl.log", string("...attaching shaders: ", geomID, "..."))
        glAttachShader(shaderProgramID, geomID)
    end
...
function createprogram(vertSource::String, fragSource::String; geomSource::String="")
    vertexShader = createshader(vertSource, GL_VERTEX_SHADER)
    fragmentShader = createshader(fragSource, GL_FRAGMENT_SHADER)
    if geomSource != ""
        geometryShader = createshader(geomSource, GL_GEOMETRY_SHADER)
        shaderProgramID = createprogram(vertexShader, fragmentShader; geomID=geometryShader)
    else
