function hungarian(input)
    A = copy(input)
    rowLen, columnLen = size(A)
    # preliminaries
    # "no lines are covered;"
    rowCovered = zeros(Bool, rowLen)
    columnCovered = zeros(Bool, columnLen)
...
function step3!(A, Zs, rowCovered, columnCovered)
